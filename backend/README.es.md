...
* Swagger UI hecho a mano en `GET /docs` (spec en `GET /openapi.json`) — sin dependencias extra.
* Reentrenamiento protegido opcionalmente por token (`RETRAIN_TOKEN`).
* Cada candidato de reentrenamiento se valida contra un holdout temporal (MAE) antes de publicarse.
...
[app/README_reentrenamiento.md](app/README_reentrenamiento.md) — se mantiene
pegado al código, así que puede ir por detrás de la descripción de más arriba.

## Variables de entorno

| Variable | Obligatoria | Descripción |
|---|---|---|
| `RETRAIN_TOKEN` | No | Si está definida, protege `POST /retrain` y `POST /retrain/reset` mediante la cabecera `X-Retrain-Token`. Está comentada por defecto en `render.yaml`, porque activarla hace que la página de Reentrenar del frontend reciba `401` (el navegador no tiene dónde guardar el token de forma segura). No hay `.env.example` para el backend: se define como variable de entorno normal en local, o descomentándola en `render.yaml` para el despliegue. |
