...
flowchart LR
    ...
    subgraph Render["Render (Blueprint)"]
        ...
        Backend["API Backend<br/>Flask + gunicorn<br/>(Web Service)"]
        Modelos[("Artefactos del modelo (.joblib)<br/>original + reentrenado")]
        Backend -->|"carga para /predict"| Modelos
        Backend -->|"POST /retrain escribe"| Modelos
    end
...
Los artefactos del modelo viven en el disco efímero del plan gratuito de Render — un modelo reentrenado se pierde al reiniciar/dormir el servicio y se vuelve al artefacto versionado en el repositorio. Ver [Despliegue en Render](#despliegue-en-render) para más detalle.
...
├── Endpoints.md        Referencia completa de la API (todas las rutas, ejemplos JSON, códigos de error)
...
* **[`Endpoints.md`](./Endpoints.md)**: referencia completa de la API (todas las rutas, ejemplos JSON, tabla completa de códigos de error) — un complemento más exhaustivo a la sección de [Contrato de API](#contrato-de-api-backend) de más abajo.
...
Más detalle — incluyendo el layout de la app, la navegación y el widget flotante de estado — vive en [`frontend/README.es.md`](./frontend/README.es.md).
...
Este es un espejo del contrato canónico documentado en [`backend/README.es.md`](./backend/README.es.md); ver también [`Endpoints.md`](./Endpoints.md) para la referencia exhaustiva.
...
Proyecto realizado en equipo durante un bootcamp:
...
Este repositorio ([`Emigarsan/Spa_model_webservice`](https://github.com/Emigarsan/Spa_model_webservice)) es la copia personal de Emilio del proyecto de equipo original, conservada como pieza de portfolio.
